...
def load_organization_config():
    ...
    if token:
        ...
        if org_id:
            ...
            # Try to get additional org info from server
            try:
                org_info = get_organization_info_from_server(token, org_id)
                if org_info:
                    config.update(org_info)
                    logging.info(f"Organization info loaded from server: {org_info.get('organization_name', 'Unknown')}")
            except Exception as e:
                logging.warning(f"Failed to load org info from server: {e}")
    ...
    if os.getenv('TRACKM_SERVER_URL'):
        ...
        config['api_endpoint'] = f"{server_url}/api"
...
def get_organization_info_from_server(token: str, org_id: int) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        response = requests.get(
            f"{config.get('api_endpoint', 'http://localhost:8000/api')}/organizations/{org_id}",
            headers=headers,
            timeout=10
        )
...
class WindowsAgent:
    def __init__(self):
        ...
        self.api_url = ORG_CONFIG['api_endpoint'].replace('/api', '')
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        if set_agent_token(token):
            print("Token set successfully from command line")
        else:
            print("Failed to set token from command line")
